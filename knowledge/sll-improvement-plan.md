...
The learning loop ran 3 iterations ($5.39 spent) with zero adoptions across all agents. Root cause is a compounding chain:

- Tiny prompts (~300 tokens vs 2000 budget) give little surface area for improvement
- The proposer makes only minimal changes
- The evaluator uses coarse integer 1–5 scores causing many tied diffs
- The Wilcoxon test requires 6+ non-zero diffs from only 10 samples — making it nearly impossible to reach statistical significance

This plan fixes all five bottlenecks while preserving the PRD's requirements for quantitative rigor, compliance preservation, and audit trails.
...
- Closing protocol for both acceptance and rejection paths

---

## 2. Fix Evaluator Granularity
...
|---|---|
| <50% | **EXPAND**: substantially expand, target 70–85% of budget |
...
1. Delete existing `data/evaluations/` and `prompts/` data to start fresh
2. Run: `python -m src.learning.loop`
3. Check `iteration_1.json` — verify scores use 0.5 increments (not just integers)
4. Check that at least 1 adoption happens within first 2 iterations
5. Verify expanded prompts in `prompts/*/v1.json` show ~1000–1400 token counts
6. Verify total cost stays under $20 in `data/reports/cost_report.json`
7. Verify compliance rates remain high (>90%) across all iterations
